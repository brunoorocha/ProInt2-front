
<!-- Nav Left Component -->
<div class="col-md-3 nav-left">

    <!-- User Logged Component -->
    <div class="row no-gutters user-session">
        <div class="col">
            <img src="" class="rounded-circle" alt="">
        </div>

        <div class="w-100"></div>

        <div class="col">
            <div class="row">
                <span id="user-logged-name" class="col-10 username">Usuário Logado</span>
            </div>
        </div>

        <div class="w-100"></div>

        <div class="col">
            <span class="usertype">Nível de Acesso</span>
        </div>
    </div>

    <div class="row no-gutters nav-left-link-list">
        <ul class="nav col flex-column">

            <div class="nav-left-spacer"></div>
            <li class="nav-item link-separator">Principal</li>

            <li class="nav-item">
                <a class="principal nav-link" href="./Home.php">
                    <i class="material-icons">home</i>
                    <span>Início</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="estoque nav-link" href="./Estoque.php">
                    <i class="material-icons">view_module</i>
                    <span>Estoque</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="clientes nav-link" href="./Clientes.php">
                    <i class="material-icons">people</i>
                    <span>Clientes</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="caixa nav-link" href="#">
                    <i class="material-icons">attach_money</i>
                    <span>Caixa</span>
                </a>
            </li>

            <div class="link-separator-line"></div>
            <li class="nav-item link-separator">Mais Opções</li>

            <li class="nav-item">
                <a class="funcionários nav-link" href="./Funcionario.php">
                    <i class="material-icons">person</i>
                    <span>Funcionários</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="nav-link" href="#">
                    <i class="material-icons">pie_chart</i>
                    <span>Relatórios</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="nav-link" href="#">
                    <i class="material-icons">settings</i>
                    <span>Configurações</span>
                </a>
            </li>

            <li class="nav-item">
                <a class="nav-link" href="Login.php">
                    <i class="material-icons">eject</i>
                    <span>Sair</span>
                </a>
            </li>

        </ul>
    </div>
</div>
